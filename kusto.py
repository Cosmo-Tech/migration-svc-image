...
def create_kusto_database(
    mgmt_client: KustoManagementClient,
    database: str,
):
    args = pars_env()
    resource_group_name = args.get('DEST_RESOURCE_GROUP')
    cluster = args.get("DEST_CLUSTER")
    ...
    try:
        ...
        mgmt_client.databases.check_name_availability(
...
def iam_set(
    ...
    try:
        poller = mgmt.database_principal_assignments.begin_create_or_update(
            resource_group_name, adx_cluster_name,
            database,
...
def run_kusto(body: BodyKusto):
    ...
    for db in databases:
        ...
        if '--kusto-iam' in steps:
            iam_set(
                database=db,
                adx_cluster_name="kusto1m89t8")
        if '--kusto-create' in steps:
            create_kusto_database(database=db)
